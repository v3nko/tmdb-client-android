...
import me.venko.tmdbclient.navigation.DetailsScreen
import me.venko.tmdbclient.navigation.ForwardToDetailsRoot
import me.venko.tmdbclient.navigation.MasterScreen
...
/**
 * Tablet Navigator implementation for launch fragments and activities.<br></br>
 * Feature [BackTo] works only for fragments.<br></br>
 * Recommendation: most useful for Single-Activity application.
 */
@Suppress("TooManyFunctions")
open class TabletSupportAppNavigator(
    activity: FragmentActivity,
    private val fragmentManager: FragmentManager,
    private val masterContainerId: Int,
    private val detailsContainerId: Int
) : Navigator {

    private val activity: Activity
    private lateinit var localStackCopy: LinkedList<String?>

    constructor(activity: FragmentActivity, masterContainerId: Int, detailsContainerId: Int)
            : this(activity, activity.supportFragmentManager, masterContainerId, detailsContainerId)
    ...
    protected fun applyCommand(command: Command) {
        when (command) {
            is ForwardToDetailsRoot -> activityReplace(Replace(command.screen), command)
    ...
    protected fun fragmentForward(command: Forward) {
        ...
        val containerId = getContainerId(screen)
    ...
    protected fun getContainerId(screen: SupportAppScreen) = when (screen) {
        is MasterScreen -> masterContainerId
        is DetailsScreen -> detailsContainerId
        else -> throw IllegalStateException("Screen should implement either MasterScreen or DetailsScreen")
    }
    ...
    protected fun activityReplace(command: Replace, initiatorCommand: Command = command) {
        val screen = command.screen as SupportAppScreen
        val activityIntent = screen.getActivityIntent(activity)
        ...
        if (activityIntent != null) {
            ...
            fragmentReplace(command, initiatorCommand)
        }
    }

    protected fun fragmentReplace(command: Replace, initiatorCommand: Command = command) {
        val screen = command.screen as SupportAppScreen
        val fragment = createFragment(screen)
     ...
     */
    protected open fun unexistingActivity(screen: SupportAppScreen, activityIntent: Intent) {
     ...
     * @param screen screen
     */
    protected open fun backToUnexisting(screen: SupportAppScreen) = backToRoot()

    protected fun errorWhileCreatingScreen(screen: SupportAppScreen): Unit =
        throw IllegalArgumentException("Can't create a screen: " + screen.screenKey)
}
